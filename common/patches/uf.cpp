	...
	ENCODE(OP_Trader)
	{
		...
		switch (action) {
			...
			default: {
				...
				*p = nullptr;
				dest->FastQueuePacket(&in, ack_req);
				LogError("Unknown Encode OP_Trader action {} received.  Unhandled.", action);
			}
		}
	}
	...
	ENCODE(OP_TraderShop)
	{
		...
		switch (action) {
			...
			default: {
				...
				*p = nullptr;
				dest->FastQueuePacket(&in, ack_req);
				LogError("Unknown Encode OP_TraderShop action {} received.  Unhandled.", action);
			}
		}
	}
...
// DECODE methods
	...
	DECODE(OP_BazaarSearch)
	{
		...
		switch (action) {
			case structs::UFBazaarTraderBuyerActions::BazaarSearch: {
				...
				// set defaults to fields that aren't used in UF
	...
	DECODE(OP_Trader)
	{
		...
		switch (action) {
			...
			default: {
				LogError("Unknown Decode OP_Trader action {} received.  Unhandled.", action);
			}
		}
	}
