...
int64 NextItemInstSerialNumber = 0;
...
static int64 GetNextItemInstSerialNumber(SharedDatabase& database) {
	...
	if (free_serial_numbers == next_item_serial_number || free_serial_numbers == 0) {

		free_serial_numbers = database.GetNextGlobalItemIdLimit();
		next_item_serial_number = free_serial_numbers - SERIAL_NUMBER_RANGE;
	}

	return next_item_serial_number;
}

void EQ::ItemInstance::AddNextItemSerialNumber(std::string& new_id) {
	guids.emplace(Strings::ToUnsignedBigInt(new_id));
}

uint32 EQ::ItemInstance::GetNextItemInstSerialNumberValue() {
...
EQ::ItemInstance::ItemInstance(SharedDatabase& db, const ItemData* item, int16 charges, uint64 guid)
{
	...
	m_guid         = fmt::format("{:016}", guid ? guid : m_SerialNumber);
}
...
EQ::ItemInstance::ItemInstance(SharedDatabase *db, const ItemData* item, int16 charges)
{
	...
	m_SerialNumber = GetNextItemInstSerialNumber(*db);
	m_guid         = fmt::format("{:016}", m_SerialNumber);
}
