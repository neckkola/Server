...
void Client::BuyTraderItemByParcel(TraderBuy_Struct* tbs, const EQApplicationPacket* app) {
	...
	BaseParcelsRepository::Parcels parcel{};
	parcel.item_id = item.item_id;
	parcel.sent_date = time(nullptr);
	parcel.from_name = tbs->SellerName;
	parcel.to_name = GetName();
	parcel.quantity = tbs->Quantity;
	parcel.note = fmt::format("Thank you for your purchase of {} {}.",
		tbs->Quantity,
		tbs->ItemName
	);
...
void Client::SendParcelStatus()
{
	...
	if(p_count > 0) {
		...
		if(num_of_parcels == PARCEL_MAX_ITEMS) {
			Message(Chat::Red,
					fmt::format("You have reached the limit of {} parcels in your mailbox.  You will not be able to send parcels until you retrieve at least 1 parcel. ",
					PARCEL_MAX_ITEMS
			).c_str()
			);
		}
		else if(num_over_limit == 1) {
			MessageString(Chat::Red, 
						  PARCEL_STATUS_1, 
			...
			Message(Chat::Yellow,
					fmt::format("You have {} parcels in your mailbox. Please visit a parcel merchant soon.",
					num_of_parcels
			).c_str()
			);
		}
...
void Client::DoParcelSend(Parcel_Struct *parcel_in)
{
	...
	if(GetParcelsMap().size() >= PARCEL_MAX_ITEMS) {
		Message(Chat::Yellow,
				fmt::format("{} tells you, 'Unfortunately, I cannot send your parcel as you are at your parcel limit of {}. Please retrieve a parcel and try again.",
				merchant->GetCleanName(),
				PARCEL_MAX_ITEMS).c_str()
		);
		SendParcelAck();
		return;
	}
	...
	if(send_to_client.at(0).parcel_count >= PARCEL_MAX_ITEMS) {
		Message(Chat::Yellow, 
				fmt::format("{} tells you, 'Unfortunately, {} cannot accept any more parcels at this time. Please try again later.", 
				merchant->GetCleanName(),
	...
	switch (parcel_in->money_flag) {
        case PARCEL_SEND_ITEMS: {
			...
			if (send_to_client.at(0).character_name.empty()) {
                MessageString(Chat::Yellow, PARCEL_UNKNOWN_NAME, merchant->GetCleanName(), parcel_in->send_to,
                              inst->GetItem()->Name);
                SendParcelAck();
                return;
            }
            ...
            MessageString(Chat::Yellow, PARCEL_DELIVERY, merchant->GetCleanName(), inst->GetItem()->Name,
                          send_to_client.at(0).character_name.c_str());
        ...
        case PARCEL_SEND_MONEY: {
            ...
            MessageString(Chat::Yellow, PARCEL_DELIVERY, merchant->GetCleanName(), money.c_str(),
                          send_to_client.at(0).character_name.c_str());
