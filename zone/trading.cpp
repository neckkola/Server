...
void Client::TraderStartTrader(const EQApplicationPacket *app)
{
	...
	for (auto &i: in.items) {
		...
		//		trader_item.item_charges          = inst->GetQuantityFromCharges();
...
//
// 	for (int i = 0; i < item_limit; i++) {
// 		if ((trader_items.at(i).item_id > 0) && (trader_items.at(i).item_unique_id.compare(tpus->item_unique_id) == 0)) {
...
// 		// and do nothing.
// 		if (tpus->new_price == 0) {
// 			tpus->sub_action = BazaarPriceChange_RemoveItem;
// 			QueuePacket(app);
// 			return;
// 		}
//
...
// 			if (newgis->items[i] > 0 && newgis->serial_number[i].compare(tpus->item_unique_id) == 0) {
...
// 			tpus->sub_action = BazaarPriceChange_Fail;
// 			QueuePacket(app);
// 			TraderEndTrader();
// 			safe_delete(newgis);
//
...
// 				trader_item.item_cost             = tpus->new_price;
// 				trader_item.item_id               = newgis->items[i];
// 				trader_item.item_unique_id        = newgis->serial_number[i];
// 				trader_item.listing_date          = time(nullptr);
...
// 		tpus->sub_action = BazaarPriceChange_AddItem;
// 		QueuePacket(app);
//
// 		return;
// 	}
...
// 	//
// 	if ((old_price != 0) && (tpus->new_price > old_price) && GetCustomerID()) {
// 		tpus->sub_action = BazaarPriceChange_Fail;
// 		QueuePacket(app);
// 		TraderEndTrader();
// 		Message(
...
// 		tpus->sub_action = BazaarPriceChange_AddItem;
// 	}
// 	else if (tpus->new_price != 0) {
// 		tpus->sub_action = BazaarPriceChange_UpdatePrice;
// 	}
// 	else {
// 		tpus->sub_action = BazaarPriceChange_RemoveItem;
// 	}
...
// 	//
// 	database.UpdateTraderItemPrice(CharacterID(), id_of_item_to_update, charges_on_item_to_update, tpus->new_price);
//
...
void Client::BuyTraderItemFromBazaarWindow(const EQApplicationPacket *app)
{
	...
	auto offline        = AccountRepository::GetAllOfflineStatus(database, trader_item.character_id);
	...
	out_data->offline_purchase               = offline,
